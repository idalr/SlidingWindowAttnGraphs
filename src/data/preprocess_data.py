...
def load_data(in_path, data_train, labels_train, data_test, labels_test, with_val=False):
    """
    This function first check if there is a folder with already processed data on disk and load the dataset from there.
    Otherwise, create the dataframes from scratch and save them on the specified path.
    """
    try:
        if with_val:
        ...
        if with_val:
            print(
                "Please check that the data is in the correct folder or whether the data has been processed previously.")
            return None, None, None
        else:
            print(
                "Creating dataframes...")  # Specifically for the HND dataset (no validation set is provided by the authors)
...
def create_dataframe(in_path, data_df,
                     labels_df):  # Specifically for the HND dataset: It transforms XML files to a dataFrame
    # The XML files are downloaded from the HND dataset repository
    ...
    for article_id in dict_text_file.keys() and dict_lab_file.keys():
        if dict_text_file[article_id][1].strip() != "":
            df_retrieve.loc[len(df_retrieve)] = {"article_id": article_id,
                                                 "article_title": dict_text_file[article_id][0],
                                                 "article_text": dict_text_file[article_id][1],
                                                 "article_label": dict_lab_file[article_id]}
        else:
