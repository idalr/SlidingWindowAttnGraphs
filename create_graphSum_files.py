...
def main_run(config_file , settings_file):
    ...
    if not os.path.exists(path_root+"/raw/"+filename_train):
        ...
        with open(file_results+'_'+model_name+'.txt', 'a') as f:
            ...
            start_creation = time.time()
            filename_train = "predict_train_documents.csv"
            dataset_train = AttentionGraphs_Sum(path_root, filename_train, filter_type, loader_train, degree=tolerance, model_ckpt=path_checkpoint, mode="train", binarized=flag_binary, multi_layer_model=multi_flag)
            ...
            start_creation = time.time()
            filename_val = "predict_val_documents.csv"
            dataset_val = AttentionGraphs_Sum(path_root, filename_val, filter_type, loader_val, degree=tolerance, model_ckpt=path_checkpoint, mode="val", binarized=flag_binary, multi_layer_model=multi_flag)
            ...
            start_creation = time.time()
            filename_test = "predict_test_documents.csv"
            dataset_test = AttentionGraphs_Sum(path_root, filename_test, filter_type, loader_test, degree=tolerance, model_ckpt=path_checkpoint, mode="test", binarized=flag_binary, multi_layer_model=multi_flag)
