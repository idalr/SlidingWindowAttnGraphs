...
def main_run(config_file , settings_file):
    ...
    df_full_train, df_test = load_data(**config_file["data_paths"])
    ...
    for exec_i in range(config_file["num_executions"]):
        ...
        loader_train, loader_val, loader_test, _ , _ , _, _ = create_loaders(df_full_train, df_test, max_len, config_file["batch_size"], 
                                                                            task="classification", tokenizer_from_scratch=False, 
                                                                            path_ckpt=config_file["data_paths"]["in_path"])
        """
        #save sentence dictionary-vocabulary on disk if you're creating loaders with 'tokenizer_from_scratch=True' 
        ...
        trainer = pl.Trainer(accelerator=config_file.get("device", "gpu"), devices=1, callbacks=[early_stop_callback, checkpoint_callback], logger=wandb_logger, **config_file["trainer_args"]) 
