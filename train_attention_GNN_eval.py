...
def main_run(config_file, settings_file):
    ...
    insert_checkpoint = args.model_dir
    ...
    path_checkpoint = "/content/drive/MyDrive/ADG/HomoGraphs_ArXiv/ReLu_w30-epoch=06-Val_f1-ma=0.81.ckpt"
    model_score = 0.811
    ...
    try:
        ...
        if config_file["with_cw"] == True:
            ...
            calculated_cw = None
        df_train, df_val = df_train[:50], df_val[:50]
    except:
        pass
    ...
    with open(file_results + '.txt', 'a') as f:
        ...
        #
                    if config_file["load_data_paths"]["with_val"] == True:
                        _, _, test_loader = partitions(dataset_train, dataset_test,
        ...
        #             print(f"Training time: {train_time:.2f} secs", file=f)
                    ...
                    model = GAT_model(384, 256, num_classes, 3, lr,
                                                    dropout=dropout,
                                                    class_weights=calculated_cw)
                    checkpoint = torch.load(insert_checkpoint)
