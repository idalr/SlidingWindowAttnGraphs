    ...
    def __getitem__(self, idx):
        ...
        if self.task == "classification":
            ...
            return {'documents_ids': torch.tensor(document), 'labels': torch.tensor(label + [-1] * num_pad_values),
                    ...
                    'article_id': article_id}  ### summarization as sent-clasification -- labels with same length
...
def create_loaders(df_full_train, df_test, max_len, batch_size, with_val=True, tokenizer_from_scratch=True,
                   ...
                   sent_tokenizer=False):  ### when val partition doesn't exist in original dataset
    ...
    if with_val:  ### if val doesn't exist, split training data
        ...
        if task == "classification":
            ...
            labels = [clean_tokenization_sent(doc, "label") for doc in
                      list(df_full_train[label_column_name[task]])]  ## no debiese pasar con mis datasets
    ...
    if tokenizer_from_scratch == False:
        ...
        vocab_sent = {v: k for k, v in invert_vocab_sent.items()}  # sourcentam hioer regkado
