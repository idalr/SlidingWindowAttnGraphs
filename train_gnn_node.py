import yaml
import argparse
from graph_data_loaders import AttentionGraphs_Sum, HeuristicGraphs, UnifiedAttentionGraphs_Sum, UnifiedHeuristicGraphs
import torch
import os
import time
import pandas as pd
...
def main_run(config_file, settings_file):
    ...
    path_results = config_file["data_paths"][
        "results_folder"]  # "/home/mbugueno/GNN_Results/Summarizer_Results/Attention/"
    ...
    if config_file["baseline"] == True:
        ...
        path_root = root_graph + "Heuristic_uni/" + heuristic
        project_name = type_model + "_" + heuristic
        file_results = path_results + dataset_name + "/" + project_name
        path_models = path_logger + "Heuristic_uni/"
    ...
    try:
        if config_file["baseline"] == True:
            print("Running", type_model, "on Heuristic-based document graphs.")
            print("Loading graphs from:", path_root)
            # dataset_train = HeuristicGraphs(root=path_root, filename=filename_train, heuristic=heuristic, path_invert_vocab_sent=path_vocab, mode="train", task="summarization")
            # dataset_val = HeuristicGraphs(root=path_root, filename=filename_val, heuristic=heuristic, path_invert_vocab_sent=path_vocab, mode="val", task="summarization")
            # dataset_test = HeuristicGraphs(root=path_root, filename=filename_test, heuristic=heuristic, path_invert_vocab_sent=path_vocab, mode="test", task="summarization")
            with open(file_results + '.txt', 'a') as f:
                print("================================================", file=f)
                print("Heuristic Graph Creation/Loading Time:", heuristic, file=f)
                print("================================================", file=f)
                start_creation = time.time()
                dataset_train = UnifiedHeuristicGraphs(root=path_root, filename=filename_train, heuristic=heuristic,
                                                       path_invert_vocab_sent=path_vocab, mode="train",
                                                       task="summarization")
                ...
                start_creation = time.time()
                dataset_val = UnifiedHeuristicGraphs(root=path_root, filename=filename_val, heuristic=heuristic,
                                                     path_invert_vocab_sent=path_vocab, mode="val",
                                                     task="summarization")
                ...
                start_creation = time.time()
                dataset_test = UnifiedHeuristicGraphs(root=path_root, filename=filename_test, heuristic=heuristic,
                                                      path_invert_vocab_sent=path_vocab, mode="test",
                                                      task="summarization")
        ...
        print(
            "If learned-graphs: Please check data paths and make sure that the graph objects were correctly generated by <create_graphSum_file.py>")
        print("If heuristic-graphs: Please check parameters for graph-objects generation.")

    """       
        if config_file["baseline"]==True:
            print ("\nCreating new graph dataset from heuristic:", heuristic)
        else: 
            print ("Please check data paths and make sure that the graph objects were correctly generated by <create_graphSum_file.py>")
            return 

        #### Creating Heuristic-based graphs         
        with open(file_results+'.txt', 'a') as f:
            print ("================================================", file=f)
            print ("Heuristic Graph Creation Time:", heuristic, file=f)
            print ("================================================", file=f)  
            start_creation = time.time()
            dataset_train = UnifiedHeuristicGraphs(root=path_root, filename=filename_train, heuristic=heuristic, path_invert_vocab_sent=path_vocab, mode="train", task="summarization") 
            creation_train = time.time()-start_creation
            print ("[TRAIN] Dataset creation time: ", creation_train, file=f)    
            print ("[TRAIN] Dataset creation time: ", creation_train)  
            start_creation = time.time()
            dataset_val = UnifiedHeuristicGraphs(root=path_root, filename=filename_val, heuristic=heuristic, path_invert_vocab_sent=path_vocab, mode="val", task="summarization")
            creation_val = time.time()-start_creation
            print ("[VAL] Dataset creation time: ", creation_val, file=f)
            print ("[VAL] Dataset creation time: ", creation_val)
            start_creation = time.time()
            dataset_test = UnifiedHeuristicGraphs(root=path_root, filename=filename_test, heuristic=heuristic, path_invert_vocab_sent=path_vocab, mode="test", task="summarization")
            creation_test = time.time()-start_creation
            print ("[TEST] Dataset creation time: ", creation_test, file=f)
            print ("[TEST] Dataset creation time: ", creation_test)
    ...
    if graph_construction == "order" or graph_construction == "window" or "binary" in graph_construction:
