...
class SlidingWindowMultiHeadSelfAttention(nn.Module):
    ...
    # def get_window_size(self, src_key_padding, min_window_size=2):
    #     len_valid_sents = src_key_padding.sum(dim=1)
    #     min_len_for_sliding_window = min_window_size*100/self.window_size

    #     if len_valid_sents <= 1:
    #         return None
    #     elif 1 < len_valid_sents < min_len_for_sliding_window:
    #         return len_valid_sents# return any value bigger than avail window size
    #     elif len_valid_sents >= min_len_for_sliding_window:
    #         return torch.ceil(len_valid_sents*self.window_size/100)
    #     else:
    #         print("Wrong size for no. valid sentences")

    def create_sliding_window_mask(self, src_key_padding_mask, matrix_mask, seq_length, min_window_size=2, pad_value=-1e9):
        # TODO: sliding window masking

        #lens_valid_sent = src_key_padding_mask.sum(dim=1)
        ...
        #padding_mask = (src_key_padding_mask == 0).unsqueeze(1).expand(-1, seq_length, -1)  # [batch, seq_len, seq_len]
        padding_mask = (matrix_mask == pad_value)
        sliding_mask[padding_mask] = pad_value


        # sliding_mask = torch.full((batch_size,seq_length,seq_length), -1e9, device=matrix_mask.device)
        #padding_mask = (matrix_mask == -1e9)

        # for i in range(seq_length):
        #     start = max(0, i - self.window_size)
        #    end = min(seq_length, i + self.window_size + 1)
        #    sliding_mask[:, i, start:end] = 0

        # sliding_mask[padding_mask] = -1e9
        # return matrix_mask + sliding_mask
        return sliding_mask
