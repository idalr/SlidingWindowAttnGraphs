...
def clean_dataframe(in_dataframe, top_k_sum=False):
    ### remove duplicates
    print ("There are", in_dataframe.shape[0], "samples in the original set.")
    in_dataframe_duplicated = in_dataframe[in_dataframe.duplicated(subset=['report'])]
    print ("There are", in_dataframe_duplicated.shape[0], "duplicated samples in the partition.")
    print ("Removing duplicates...")
    in_dataframe = in_dataframe.drop_duplicates(subset=['report'])
    ...
    if top_k_sum:
        drop_sample=[True if ((len(source)-len(summary)<0) or len(source)==0 or len(summary)==0 or check_conditions_paper(source, summary)) else False for source, summary in zip(in_dataframe.report,in_dataframe.summary)]
    else: 
        drop_sample=[True if ((len(source)-len(summary)<0) or len(source)==0 or len(summary)==0) else False for source, summary in zip(in_dataframe.report,in_dataframe.summary)]
    ...
    print ("There are", in_dataframe.shape[0], "samples in the final set.")
    return in_dataframe
...
def merge_short(document_as_sentences):
    ...
    for sentence in document_as_sentences:
        ...
        if (sentence not in ["» .","'.'",'"...','\\ .','ET).','!!"','* .']) and len(sentence)>15 and not on_hold: 
            ...
            try:
                ...
                print ("\nError merging sentence:", sentence, "-- On-Hold:", on_hold, "-- len merged:", len(merged_sentences))   
...
def clean_and_save_data(in_args, R_scorer): ## clean specific document
    ...
    # dictionary of lists 
    dict_calculated_scores = {'Sentence_id': range(len(merged_source)), 'Rouge-1/2': all_fscore12, 'Sentence': merged_source} 
    ...
    for sentence_id, text, rouge in zip(df_calculated_scores.Sentence_id, df_calculated_scores.Sentence, df_calculated_scores["Rouge-1/2"]):
        if len(selected)==0:
            ...
            if num_WordsInSelected >= num_WordsInSummary:
                ...
                current_score = average_rouge(summary, cumulative, R_scorer) #R_scorer.score(summary, cumulative)['rouge1'].fmeasure
...
def clean_and_save_data_best_increment(in_args, R_scorer): ## clean specific document
...
def create_cleaned_objects_best_increment(df_input, partition, with_RL=False):
    ...
    with mp.get_context("fork").Pool(processes=len(os.sched_getaffinity(0))) as pool:
        res = pool.imap(partial(clean_and_save_data_best_increment, **{"R_scorer": R_scorer}), 
                                                        list(zip(original_indexes, df_input.report, df_input.summary)))
