...
##llamar con loader usando batch size 1
class MP_UnifiedAttentionGraphs_Sum(Dataset):
    def __init__(self, root, filename, filter_type, data_loader, degree=0.5, model_ckpt="", mode="train",
                 transform=None, normalized=False, binarized=False, multi_layer_model=False,
                 pre_transform=None):  # input_matrices, invert_vocab_sent
        ### df_train, df_test, max_len, batch_size tambien en init?
        ...
        super(MP_UnifiedAttentionGraphs_Sum, self).__init__(root, transform, pre_transform)
...
class PartialPTDataset(Dataset):
    ...
    def __init__(self, root, mode='train', transform=None, pre_transform=None):
        ...
        self.pt_files = self._load_existing_pt_files()

    def _load_existing_pt_files(self):
        processed_dir = os.path.join(self.root, 'processed')
        all_files = os.listdir(processed_dir)

        # Build regex pattern based on mode
        if self.mode == 'train':
            pattern = r'data_(\d+)\.pt'
        else:
            pattern = rf'data_{self.mode}_(\d+)\.pt'

        # Filter and sort files by extracted numeric index
        matched_files = [
            f for f in all_files if re.fullmatch(pattern, f)
        ]
        return sorted(matched_files, key=lambda x: int(re.findall(r'\d+', x)[0]))
    ...
    def get(self, idx):
        file_path = os.path.join(self.processed_dir, self.pt_files[idx])
