...
def main_run(config_file , settings_file):
    ...
    try:
        if config_file["baseline"]:
            ...
            print ("Loading graphs from:", path_root)
            dataset_train = AttentionGraphs_Sum(root=path_root, filename=filename_train, filter_type=type_graph, data_loader=None, mode="train", binarized=flag_binary, multi_layer_model=multi_flag)
            dataset_val = AttentionGraphs_Sum(root=path_root, filename=filename_val, filter_type=type_graph, data_loader=None, mode="val", binarized=flag_binary, multi_layer_model=multi_flag)
            dataset_test = AttentionGraphs_Sum(root=path_root, filename=filename_test, filter_type=type_graph, data_loader=None, mode="test", binarized=flag_binary, multi_layer_model=multi_flag)
            print ("Graphs correctly loaded.")
    ...
    with open(file_results+'.txt', 'a') as f:
        try:
            ...
            _, loader_val, loader_test, _ , _ , _, _ = create_loaders(df_train, df_test, max_len, config_file["batch_size"], df_val=df_val, 
                                                                      task="summarization", tokenizer_from_scratch=False, path_ckpt=path_vocab)
        ...
        for nl in n_layers: # 1, 2, 3
            for dim in dim_features: # 64, 128 o 256
                ...
                for i in range(num_runs):  
                    ...
                    if type_model=="GAT":
                        model = GAT_NC_model(dataset_train.num_node_features, dim, num_classes,  nl, lr, dropout=dropout, class_weights=calculated_cw, with_edge_attr=with_edge_attr)
                    ...
                    if config_file["baseline"]:
                        wandb_logger = WandbLogger(name=type_model+"_"+str(nl)+"L_"+str(dim)+"U_"+graph_construction+"_run"+str(i), save_dir=path_for_savings, project=project_name)
                    else:
                    ...
                    train_loader, val_loader, test_loader = partitions(dataset_train, dataset_test, dataset_val=dataset_val, bs=config_file["batch_size"]) ## try a small number 8-16
                    starti = time.time()
