...
from graph_data_loaders import ActualUnifiedAttentionGraphs_Sum, UnifiedAttentionGraphs_Sum, \
    PartialPTDataset  # , AttentionGraphs_Sum
import torch
...
def main_run(config_file, settings_file):
    ...
    if unified_flag == True:
        # dataset_train = ActualUnifiedAttentionGraphs_Sum(root=path_root, filename=filename_train, filter_type=type_graph,
        #                                            data_loader=None, mode="train",
        #                                            binarized=flag_binary, multi_layer_model=multi_flag)
        # dataset_val = ActualUnifiedAttentionGraphs_Sum(root=path_root, filename=filename_val, filter_type=type_graph,
        #                                          data_loader=None, mode="val",
        #                                          binarized=flag_binary, multi_layer_model=multi_flag)
        # dataset_test = ActualUnifiedAttentionGraphs_Sum(root=path_root, filename=filename_test, filter_type=type_graph,
        #                                           data_loader=None, mode="test",
        #                                           binarized=flag_binary, multi_layer_model=multi_flag)
        # call existing data
        dataset_train = PartialPTDataset(root=path_root, mode='train')
        dataset_val = PartialPTDataset(root=path_root, mode='val')
        dataset_test = PartialPTDataset(root=path_root, mode='test')
    ...
    ids2remove_train = check_dataframe(df_train[:MAX_TRAIN_INDEX])
        ...
        for nl in n_layers:  # 1, 2, 3
            for dim in dim_features:  # 64, 128 o 256
                ...
                for i in range(num_runs):
                    ...
                    train_loader, val_loader, test_loader = partitions(dataset_train[:MAX_TRAIN_INDEX], dataset_test,
