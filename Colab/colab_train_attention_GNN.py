...
def main_run(config_file , settings_file):
    ...
    path_checkpoint = os.path.normpath(root_graph+model_name+model_version)
    ...
    try:
        ...
        load_data_train_path = os.path.normpath(config_file["data_paths"]["in_path"]+"/Processed/df_train.csv")
        ...
        load_data_train_path = os.path.normpath(config_file["data_paths"]["in_path"] + "/Processed/df_train.csv")
        df_full_train = pd.read_csv(load_data_train_path)
        load_data_test_path = os.path.normpath(config_file["data_paths"]["in_path"] + "/Processed/df_test.csv")
                ...
                for i in range(num_runs):
                    ...
                    if config_file["normalized"]:
                        checkpoint_callback = ModelCheckpoint(monitor="Val_f1-ma", mode="max", save_top_k=1, dirpath=path_for_savings,
                                                              filename=type_model+"_"+str(nl)+"L_"+str(dim)+"U_"+type_graph+"_norm_run"+str(i)+"-OUT-{epoch:02d}-{Val_f1-ma:.2f}")
