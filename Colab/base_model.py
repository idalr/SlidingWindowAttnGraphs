...
class SlidingWindowMultiHeadSelfAttention(nn.Module):
    def __init__(self, input_dim, embed_dim, num_heads, window_size, temperature=1, dropout=0.0,
    ...
    def create_sliding_window_mask(self, src_key_padding_mask, matrix_mask, seq_length, min_window_size=2,
                                   pad_value=-1e9):
        ...
        window_mask = (idx.view(1, -1, 1) - idx.view(1, 1, -1)).abs() > window_sizes.view(-1, 1,
                                                                                          1)  # if masked window, then True
...
class MHAClassifier(Classifier_Lighting):
    def __init__(self, embed_dim, num_classes, hidden_dim, max_len, lr, window,
        ...
        # sliding window MHA
            ...
            self.attention = SlidingWindowMultiHeadSelfAttention(self.sent_model.get_sentence_embedding_dimension(),
                                                                 embed_dim, num_heads, window_size=self.window,
                                                                 temperature=1,
    ...
    def forward(self, doc_ids, src_key_padding_mask, matrix_mask):
        ...
        attn_output = torch.nanmean(attn_output,
                                    dim=1)  # Mean pooling over the sequence length  #### que pasa con el masking de las no sentencias? para calcular loss  -- necesito recuperar el max length del documento
...
class MHAClassifier_extended(Classifier_Lighting):
    def __init__(self, embed_dim, num_classes, hidden_dim, max_len, lr,
        ...
        if intermediate:
            ...
            self.multi_attention = MultiHeadSelfAttention(hidden_dim, hidden_dim, num_heads, temperature=1,
                                                          dropout=attn_dropout,
                                                          activation_attention=activation_attention)
    ...
    def forward(self, doc_ids, src_key_padding_mask, matrix_mask):
        ...
        if self.multi_layer:
            attn_output, attn_weights = self.multi_attention(attn_output.float().to(self.device), src_key_padding_mask,
                                                             matrix_mask, temperature=self.temperature)
